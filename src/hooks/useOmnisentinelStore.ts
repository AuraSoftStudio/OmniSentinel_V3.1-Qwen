...
// import { persist } from 'zustand/middleware'; // Desactivado temporalmente para debug
...
// 🔥 IMPORTACIÓN ESTÁTICA DEL TEMPLATE (Más seguro)
...
// 🔥 CREACIÓN DEL STORE SIN PERSISTENCIA TEMPORALMENTE
export const useOmnisentinelStore = create<OmnisentinelState>((set, get) => ({
  ...
  init: () => {
    console.log("🚀 [STORE] Iniciando aplicación...");
    const { rawNodes } = get();
    
    // Si no hay nodos, cargamos el template estático directamente
    if (rawNodes.length === 0) {
      console.log("📦 [STORE] No hay datos. Cargando template por defecto...");
      // Casting necesario porque el JSON importado es 'any'
      get().loadCustomNodes(logisticsTemplate as unknown as ProjectNode[]);
    } else {
  ...
  loadCustomNodes: (nodes: ProjectNode[]) => {
    ...
    set({ rawNodes: nodes, isLoading: true, dataSource: 'TEMPLATE' });
    get().recalculate().finally(() => {
      console.log("✅ [STORE] Cálculo finalizado. UI lista.");
      set({ isLoading: false });
    });
  },
  ...
  recalculate: async () => {
    ...
    if (rawNodes.length === 0) {
      console.warn("⚠️ [V-CORE] No hay nodos para calcular.");
      return;
    }
    ...
    try {
      ...
      console.log("✅ [V-CORE] Cálculo exitoso. Nodos procesados:", result.nodes.length);
    } catch (err) {
  ...
  toggleKillSwitch: (nodeId: string) => {
    console.log("🔌 [KILL SWITCH] Toggle en nodo:", nodeId);
    const { rawNodes } = get();
